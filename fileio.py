...
class spctfile(object):
    def read_data(self):
        return np.loadtxt(self.name,use_cols=(0,2))
        return array(datafile.read_data(self, sel_columns=[0,2]))
class ststfile(file):
    def _readWParam(self):
        print
    def getWParams(self):
        wStart='        XS        VS          LOG RH       TE          TR         W'
        wEnd='     No. of ions =  '
        wEndAlternative='         En consv. check'
        doYield=False
        rowFormat={'names':('xs','vs','logrh','te','tr','w'),
        ...
        for line in self:
            ...
            if line.startswith(wStart):
                doYield=True
                continue
            if line.startswith(wEnd) or line.startswith(wEndAlternative):
                if doYield:
                    wParams.append(np.array(tmpParam,dtype=rowFormat))
                    tmpParam=[]
                doYield=False
                continue
            if doYield:
                tmpParam.append(tuple(line.strip().split()))
        return wParams
def get_temp(data):
...
def createSNDirStructure(SNName,path='.'):
    SNDir = os.path.join(path,SNName)
    os.makedirs(SNDir)
    SNSubDirs = ['spectra','manual','auto','sh_fits']
    for dir in SNSubDirs:
        os.makedirs(os.path.join(SNDir,dir))
